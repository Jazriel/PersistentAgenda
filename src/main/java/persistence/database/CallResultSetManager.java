...
public class CallResultSetManager extends ABCResultSetManager<Call> {
	...
	private Call getCallFromResultSet(ResultSet rs) {
		Call call = null;
		try {
			Contact contact = new FactoryDataBase().createContactPersistence().getContactById(rs.getInt(2));
			call = new Call(rs.getInt(1), contact, rs.getDate(3).toString(), rs.getString(4), rs.getString(5));
